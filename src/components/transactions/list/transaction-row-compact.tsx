...
export default function TransactionRowCompact({ transaction }: TransactionRowCompactProps) {
    ...
    // Clean description (remove "(Copy)" suffix for display)
    const cleanDescription = transaction.description?.replace(/\s*\(Copy\)\s*$/i, '') || 'Untitled'

    return (
        <div className="group flex items-center gap-3 px-4 py-2.5 hover:bg-elevated/20 transition-colors duration-150">
            {/* Category Icon */}
            <div className={`flex items-center justify-center w-8 h-8 rounded-lg shrink-0 ${isExpense ? 'bg-elevated' : 'bg-positive/10'
                }`}>
                <IconComponent
                    size={14}
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-tertiary truncate">
                    {transaction.categories?.name || 'Uncategorized'} • {transaction.accounts?.name || 'Unknown'}
                </p>
            </div>

            {/* Amount & Time */}
            <div className="flex flex-col items-end shrink-0">
                <p className={`text-sm font-semibold data-text ${isExpense ? 'text-negative' : 'text-positive'
                    }`}>
                    ...
                    <>
                        ...
                        <div className="absolute right-0 top-8 z-50 w-32 rounded-lg border border-border bg-surface py-1 shadow-xl">
                            <button
                                ...
                                className="flex items-center gap-2 w-full px-3 py-2 text-xs text-muted hover:text-foreground hover:bg-elevated transition-colors duration-150"
                            >
                                <Edit2 size={12} />
                                Edit
                            </button>
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 w-full px-3 py-2 text-xs text-negative hover:bg-negative/10 transition-colors duration-150"
                            >
                                <Trash2 size={12} />
                                Delete
                            </button>
